...
namespace CUESYSv._01
{
    public class dbConn
    {
        ...
        public void insertBooking(string custContact, string Airline, string Origin, string Destination, string FlightNumber, string bookingDate, string bookingTime, string bookingCost, string bookingPaid)
        {
            MySqlCommand comm = conn.CreateCommand();
            comm.CommandText = "INSERT INTO `tblBookings` (`custContact`, `Airline`, `Origin`, `Destination`, `FlightNumber`, `bookingDate`, `bookingTime`, `bookingCost`, `bookingPaid`) VALUES (@custContact, @Airline, @Origin, @Destination, @FlightNumber, @bookingDate, @bookingTime, @bookingCost, @bookingPaid);";
            comm.Parameters.AddWithValue("@custContact", custContact);
        ...
        {
            MySqlCommand comm = conn.CreateCommand();
            comm.CommandText = "INSERT INTO `tblBookings` (`fligtStatus`, `displayCost`, `bookingFloor`, `bookingUpdate`, `bookingDateTime`, `bookingCost`, `bookingPaid`) VALUES (@custContact, @bookingBuilding, @bookingFloor, @bookingRoom, @bookingDateTime, @bookingCost, @bookingPaid);";
            comm.Parameters.AddWithValue("@custContact", custContact);
